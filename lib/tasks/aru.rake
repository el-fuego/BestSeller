...
namespace :aru do
  ...
  task :parse, [:start_page, :page_count] => [:environment] do |t, args|
    args.with_defaults(start_page: 0, page_count: 3)
    ...
    (startPage..endPage).to_a.each do |page_id|
      Parallel.each(loadAdvertsIds(page_id), in_theads: 20) do |advert_id|
  ...
  def saveAdvert(page, id)
    ...
    if advert_exist? page
      #puts "page exist at #{_url}"
      return nil
    end
  ...
  def _get_last_advert_date
    if !@last_advert_date
      # do not parse exist adverts
      last_advert = Advert.limit(1)[0]
      if last_advert
        @last_advert_date = last_advert.advert_created_at.to_datetime
      else
        @last_advert_date = DateTime.parse('2001-01-01T00:00:00')
      end
    end
  ...
  def advert_exist? (page)
    date = page.css('.date-add')
    date && date.length > 0 && (DateTime.parse(date.first.attr('pvalue')) < _get_last_advert_date)
  end
end